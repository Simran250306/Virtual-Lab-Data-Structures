...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($fullname === '' || $username === '' || !filter_var($email, FILTER_VALIDATE_EMAIL) || $college === '' || $department === ''
        || $rawPassword === '' || !in_array($securityQuestion, $allowedQuestions, true) || $securityAnswer === '') {
    ...
    $checkStmt = $conn->prepare('SELECT id FROM student_table WHERE username = ? OR email = ? LIMIT 1');
    if ($checkStmt === false) {
        echo 'Query error.';
        exit();
    }
    ...
    try {
        $insertUserStmt = $conn->prepare('INSERT INTO student_table (name, username, email, college, department, password) VALUES (?, ?, ?, ?, ?, ?)');
        if ($insertUserStmt === false) {
            throw new Exception('User insert prepare failed');
        }
        $insertUserStmt->bind_param('ssssss', $fullname, $username, $email, $college, $department, $password);
        ...
        header('Location: ../login.html?signup=success&verify=' . urlencode($verifyStatus));
        exit();
    } catch (Exception $e) {
        $conn->rollback();
